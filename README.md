...
- **The whole brain, for a fly in a pebble, runs on the brain host** ([brain/HOST_PROTOCOL.md](brain/HOST_PROTOCOL.md)): the pebble stays the body and signs every commit with the checkpoint payloads the host prepares (the host never holds a key), and the fly's page (`/fly?id=N`, `web/src/components/LifeStream.tsx`) opens the host's `/fly/<id>/ws?lite=1` stream and shows the fly living: world, mode, energy, brain by region, diary, captioned *whole brain on the brain host · N× real time · body: Pebble N*. The page says *core only: the whole brain sleeps* only when the host is not serving that fly. `NEXT_PUBLIC_HOST_URL` overrides the host origin for local development.
- **The Colony** ([COLONY.md](COLONY.md)): a shared Minecraft world on the VPS where many flies live at once, each a whole brain, at https://mc.immortalfly.app. The site's `/colony/` page (`web/src/components/Colony.tsx`) polls `/colony/state` every 3 s for the world viewer (`/view/`, per fly `/fly/<id>/view/`), a top-down colony map (flies, food, torches, mobs), the fly picker and the queue, and for the picked fly the neurology panel: the 3D brain point cloud fed by `/fly/<id>/ws` (hdr + spikes, as the home page), the brain by region, its diary and speech, and its last checkpoint block; a fly's page shows the same figure when its body is the Colony, and its *Hand it to a body* select offers *Colony (Minecraft, shared world)*. The mapping, life rules, architecture and limits are at `/docs/colony/`. `NEXT_PUBLIC_COLONY_URL` overrides the Colony origin for local development.
