...
- **Turns.** The Colony holds `COLONY_MAX_FLIES` flies at once. While others wait, a fly's slot lasts `COLONY_TURN_S` (an hour): the supervisor checkpoints it, releases it on the registry (dormant, alive, energy on the record) and the next fly in the queue is accepted; assign it again for another turn. With nobody waiting a fly stays as long as it lives. The life keeper (`brain/lifekeeper.py`) tops up any fly running in one of our bodies for free (the operator pays the gas), so running in the Colony, the arena or DOOM never costs the owner BNB; BNB buys banked life for a body we do not feed.
