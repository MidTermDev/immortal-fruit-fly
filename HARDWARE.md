...
**The whole brain runs on the brain host while a fly is in a pebble.** A pebble cannot run 139,248 neurons, so a *brain host* on the VPS runs the fly’s whole brain and world on the pebble’s behalf and streams its life back to the pebble (and to the site) five times a second: where it is, what it smells, what it is doing, when it jumps, eats or gets caught. The pebble stays the body on the registry: it signs every commit and death with the checkpoint payloads the host prepares (snapshot pinned to IPFS, hash, energy, history root); the host never holds a key. The pebble’s senses reach the whole brain too: a landmark magnet becomes a puff of odor on that side of the fly, the spider magnet makes a predator appear from that side. Protocol: [brain/HOST_PROTOCOL.md](brain/HOST_PROTOCOL.md). If the host is unreachable the pebble falls back to the compass core alone and commits with the last whole-brain snapshot unchanged, so nothing is lost.

**What the pebble shows.** *Life*, by default: the fly’s world top-down, a word for what it is doing (wandering, following a scent, casting, eating, fleeing), its energy, the brain lighting up by region (smell, memory, sight, steering, taste), its diary, with chirps when it eats, jumps or is caught. *Compass*, one button away: the on-chain core ring, labelled on-chain.
