...
export default function Page() {
  return (
    <DocsShell current="/docs/play/">
      ...
      <div className="callout"><b>You need:</b> a wallet on BNB Smart Chain (MetaMask, Rabby, Binance Wallet) and a little BNB. Keeping a fly alive costs a little BNB, about 0.01 per day, and so does resurrecting one; nothing is burned for metabolism. Minting burns 1 <code>$FLY</code>, breeding 5,000. <a href={CFG.links.pancake + CFG.token}>Get $FLY on PancakeSwap</a>.</div>
      <h2>Your fly</h2>
      <table className="data"><thead><tr><th>Action</th><th>What happens</th><th>Cost</th></tr></thead><tbody>
        ...
        <tr><td>Keep alive</td><td>Pay the <a href={`${CFG.explorer}/address/${CFG.registry}#code`}>registry</a> a little BNB for any fly, yours or not (<code>feed(id, seconds)</code>, payable). The life lands on its record at once; in the arena or the Colony that is food dropped near the fly at the body&apos;s next poll, and it has to smell its way there. A fly that is not running does not age and banks it. Nothing is burned; the BNB goes to the treasury, and the operator&apos;s keeper feeds for free.</td><td>0.01 BNB ≈ 24 h</td></tr>
        <tr><td>Resurrect</td><td>Only when dead. The same brain continues from exactly the state it died in, generation + 1, with the life you give it (at least 60 s). Until then the token cannot be transferred or sold.</td><td>0.002 BNB + life, in BNB</td></tr>
