...
export const metadata: Metadata = { title: "Pebbles: the fly in hardware", description: "Five handheld M5Stack CoreS3 devices, each a wallet that owns a fruit fly and the body that runs its on-chain compass neurons, with the whole brain running for it on the brain host. The demo, the architecture, the wiring, the gas, and the bring-up." };
...
export default function Page() {
  return (
    <DocsShell current="/docs/pebbles/">
      ...
      <p><strong>The whole brain does not stop while a fly is in a pebble.</strong> It runs on the <em>brain host</em>, described next, which prepares every checkpoint for the pebble to sign; the fly&apos;s page then says <i>body: Pebble 3 · whole brain on the brain host</i> and shows its life. Only when the host is unreachable does the pebble fall back to the core alone: it commits with the fly&apos;s existing <code>stateRoot</code> and <code>stateURI</code> unchanged (the whole-brain snapshot is preserved exactly), <code>brainStep</code> advanced by the core steps it ticked, and <code>energy</code> decremented by real seconds, and the page says <i>body: Pebble 3 (core only: the whole brain sleeps)</i> until the host, or a whole-brain body such as the arena, takes it up again from that snapshot.</p>

      <h2>Where the whole brain runs</h2>
      <p>A pebble is the fly&apos;s <em>body</em> on the registry: its address accepts the fly, commits, logs interactions and reports its death, and it signs every one of those transactions with its own key. What it cannot do is run 139,248 neurons. So a <strong>brain host</strong> on the VPS runs the fly&apos;s whole brain and its world on the pebble&apos;s behalf, exactly as the arena instantiates a fly: fetch the last committed snapshot from IPFS, check that its sha256 is the <code>stateRoot</code>, load brain and world, and apply feeds and resurrections from the chain. The host is itself a registered body, <i>Brain host</i>, whose <code>uri</code> is its current public origin; a fly <code>id</code> hosted for a pebble lives at <code>&lt;uri&gt;/fly/&lt;id&gt;/…</code>, and a supervisor starts one whole-brain process for every living fly whose body is a pebble. The pebble opens <code>/fly/&lt;id&gt;/ws?lite=1</code> and receives the fly&apos;s life five times a second: where it is, what it smells, what it is doing, when it jumps, eats or is caught. The fly&apos;s page on this site opens the same stream. The pebble&apos;s senses go the other way: a landmark magnet becomes a puff of odor on that side of the fly, the spider magnet a predator approaching from that side.</p>
      <p>The host never holds a key. When it is time to commit, the pebble asks the host for a checkpoint, proving it is the body by signing a timestamped digest with the same key it registered with; the host saves the snapshot, pins it and fresh token metadata to IPFS, and returns the payload (<code>stateRoot</code>, <code>memoryRoot</code>, <code>stateURI</code>, <code>metadataURI</code>, <code>brainStep</code>, <code>energy</code>, <code>historyRoot</code> and the notable events since the last one). The pebble sends <code>commit(…)</code> and the <code>interaction(…)</code>s itself, and <code>died(…)</code> from the host&apos;s final payload when the frame says the fly is dead. The chain sees only the pebble; the host is a compute service the pebble reads from and the site watches, and it can be replaced by anyone running the same simulator. The pebble&apos;s screen says <i>brain on host · 0.8× real time</i> so nobody mistakes where the neurons are, and its compass view says <i>on-chain</i>. If the host is unreachable the pebble keeps the core running locally and commits the core-only payload as above; if the pebble is unreachable the host keeps the fly living, and the next checkpoint the pebble fetches carries the truth. The protocol, endpoints and payloads are in <a href={HOST}>brain/HOST_PROTOCOL.md</a>.</p>
