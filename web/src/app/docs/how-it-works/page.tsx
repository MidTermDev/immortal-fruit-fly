...
export default function Page() {
  ...
  return (
    <DocsShell current="/docs/how-it-works/">
      ...
      <p>One simulated second costs one second of energy. Eating restores it. Every ten minutes the server hashes the complete brain state (every membrane potential, synaptic drive, delayed input and refractory clock), saves a snapshot that also carries the full world state and the brain step at which each on-chain event was applied, and calls <code>FlyWorld.checkpoint()</code> on BNB Smart Chain with the hash, position, energy and spike count. Food exists only through <code>FlyWorld.placeFood()</code>: the server reads the event and puts the food into the arena. At zero energy the server reports the death with a final hash and keeps retrying until the contract agrees; <code>resurrect()</code> burns $FLY and the same brain continues from that snapshot with at least 30 seconds of life. To verify a checkpoint: download two consecutive snapshots and run <code>brain/verify.py A.npz B.npz</code>; it replays A through the recorded events to B&apos;s step and compares the hash. The kernel avoids fast-math so the replay is bit-exact across machines. Snapshots are served from the operator&apos;s server while it is up; the connectome file it needs is rebuilt by <code>brain/build_connectome.py</code> from the public FlyWire release.</p>
