...
from flysim import Circuit, FlyBrain, CH_CUE, CH_TURN_LEFT, CH_SHOCK
...
def run_protocol(p, want_trace=False):
    ...
    m['rateC'] = rate('C'); m['concC'] = conc('C'); m['movedC'] = 1 - cos(wins['B4'][0], wins['C'][0])
    m['rateD'] = rate('D'); m['concD'] = conc('D'); m['simD'] = cos(wins['C'][0], wins['D'][0])
    ...
    score += 3 * atleast(m['concB'], 0.75) * alive
    score += 2 * m['simB'] * alive
    score += 2 * m['movedC'] * atleast(m['concC'], 0.6) * alive * (1 if m['rateC'] >= 1 else 0)
    score += 1 * m['simD'] * atleast(m['concD'], 0.7) * (1 if m['rateD'] >= 1 else 0)
