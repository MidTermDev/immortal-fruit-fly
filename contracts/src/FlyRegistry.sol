...
contract FlyRegistry is ERC721, IERC2981 {
    ...
    uint256 public immutable FEED_PER_SECOND;
    uint256 public immutable BREED_PRICE;
    ...
    /// @notice Burn $FLY to give a fly `seconds_` more life. Its body applies it at the next poll.
    function feed(uint256 id, uint64 seconds_) external {
        ...
        uint256 cost = uint256(seconds_) * FEED_PER_SECOND;
        f.energy += seconds_;
        _burn_(msg.sender, cost);
    ...
    ///         once a body is assigned and instantiates it. Burns RESURRECT_PRICE + seconds_ * FEED_PER_SECOND.
    function resurrect(uint256 id, uint64 seconds_) external {
        Fly storage f = _flies[id];
        _requireOwned(id);
        if (f.alive) revert NotDead();
        if (seconds_ < 60) revert TooLittle();
        uint256 cost = RESURRECT_PRICE + uint256(seconds_) * FEED_PER_SECOND;
        f.alive = true;
        f.generation += 1;
        f.energy = seconds_;
        _burn_(msg.sender, cost);
